...
// Clear the screen and draw the frame
  ...
  int drawScreen2 = 0;
  for(drawScreen2=0; drawScreen2 <= 5; drawScreen2++)
  {
  i = drawScreen2;
           ...
           /* Serial.print("Time delta = ");
            ...
            for (i = 0; i < count; i++)
            {
              if(page==1){
               ...
               else if(page==6){ //270, 250, 530, 330
                 if(touchLocations[i].x>=270&&touchLocations[i].x<=530&&touchLocations[i].y>=250&&touchLocations[i].y<=330 )
                 {             
                    Home_Page(); 
                 }
               }
